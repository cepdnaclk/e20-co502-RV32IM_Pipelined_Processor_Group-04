...
# saving data to a .bin and .hex file
def saveToFile(line):
    global inst_count

    # Determine file names
    bin_file = "./" + argList['inp_file'].split('.')[0] + '.bin'
    hex_file = "./" + argList['inp_file'].split('.')[0] + '.hex'
    if not (argList['out_file'] == ''):
        base_name = "./" + argList['out_file']
        bin_file = base_name + ".bin"
        hex_file = base_name + ".hex"

    # Write to the binary file
    with open(bin_file, "a") as bin_f:
        bin_f.write(line + "\n")
    
    # Convert binary to hexadecimal and write to the hex file
    hex_line = hex(int(line, 2))[2:].zfill(8)  # Convert binary to 8-digit hex
    with open(hex_file, "a") as hex_f:
        hex_f.write(hex_line + "\n")
